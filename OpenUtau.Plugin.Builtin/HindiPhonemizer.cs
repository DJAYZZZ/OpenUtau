...
namespace OpenUtau.Plugin.Builtin {
    ...
    /// </para>
    /// </summary>
    [Phonemizer("Hindi Phonemizer", "HINDI", language: "HI")]
    public class HindiPhonemizer : LatinDiphonePhonemizer {
        public HindiPhonemizer() {
            try {
                Initialize();
            } catch (Exception e) {
        ...
        protected override IG2p LoadG2p() {
            ...
            string path = Path.Combine(PluginDir, "Hindi.yaml");
            if (!File.Exists(path)) {
                Directory.CreateDirectory(PluginDir);
                File.WriteAllBytes(path, Data.Resources.hindi_template);
            }
            g2ps.Add(G2pDictionary.NewBuilder().Load(File.ReadAllText(path)).Build());

            // Load dictionary from singer folder.
            if (singer != null && singer.Found && singer.Loaded) {
                string file = Path.Combine(singer.Location, "Hindi.yaml");
                if (File.Exists(file)) {
                    try {
